-- ========================================================================================
-- TYM2MUV FULL DATABASE SCHEMA (SUPABASE POSTGRESQL)
-- Includes Core, Agent System, Admin Dashboard, Monetization Ads, Analytics & RLS
-- ========================================================================================
...
-- ==========================================
-- STEP 4: ADMIN, DASHBOARDS & SYSTEM TABLES
-- ==========================================

-- 10. Reports (Fraud / Abuse)
...
-- 14. Property Views (Tracks individual views for analytics)
...
SELECT 
    ...
    (SELECT COUNT(*) FROM public.property_views pv JOIN public.properties p ON pv.property_id = p.id WHERE p.agent_id = a.id) AS total_views,
    (SELECT COUNT(*) FROM public.rental_requests rr JOIN public.properties p ON rr.property_id = p.id WHERE p.agent_id = a.id) AS total_requests
FROM public.agents a;

-- ==========================================
-- STEP 6: SECURITY FUNCTIONS & LOGIC
-- ==========================================
...
CREATE POLICY "Agents can view stats for own properties" ON public.property_views FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.properties p WHERE p.id = property_views.property_id AND p.agent_id = auth.uid()) OR public.is_admin()
);
